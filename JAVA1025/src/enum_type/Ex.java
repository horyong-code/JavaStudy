...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 * - enum 타입은 내부적으로 java.lang.Enum 클래스를 상속 받음
		 * 
		 * < 열거타입 정의 방법>
		 * [접근제한자] enum 열거타입명 {
		 ...
		 * - switch문 사용 가능 (단, case문 에서 열거타입명 없이 상수만 지정 필수)
		 * 	 switch(열거타입 변수){
		 *      case 열거타입 상수1 : 수행할 작업들;
		 *      case 열거타입 상수2 : 수행할 작업들;
		 *      ...;
		 *      case 열거타입 상수n : 수행할 작업들;
		 */
		
		Week week = new Week();
		
		week.setMyWeek(Week.WEEK_WEDNESDAY);
		...
		// => 상수 데이터들이 정수일 경우 해당 정수를 전달하는 메서드 (Setter) 호출 시
		//    상수 범위 값이 아닌 다른 값을 전달해도 컴파일 에러 (문법적 오류)가 발생하지 않음
		...
		mc.setMyWeek(EnumWeek.WEDNESDAY);
		System.out.println("오늘의 요일 : " + mc.getMyWeek());
		
		// 열거 타입 사용 시 장점
		// => 정의 시 지정된 상수 이외에 다른 값은 절대 전달 불가능
//		mc.setMyWeek(5);
		// => The method setMyWeek(EnumWeek) in the type MyCalendar is not applicable for the arguments (int)
//		mc.setMyWeek("WEDNESDAY");
//		mc.setMyWeek(WEDNESDAY);
...
enum EnumWeek{
	...
	// (별도의 값을 지정하지 않는다!)
	MONDAY, TUESDAY, WEDNESDAY, THURSDAY, FRIDAY, SATURDAY, SUNDAY // 상수명 -> 인스턴스 (heap) = 값
}
...
//		this.myWeek = myWeek;
		...
		// 요일에 대한 정상 범위 판별없이 저장 작업을 수행할 경우
