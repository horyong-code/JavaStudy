...
//		Date d2 = new Date(0); // 1970년 1월 1일 09시 기준 설정
		...
		// => 목표 날짜 - 현재 날짜 > 0일 경우, 결과값은 남은 일 수 (미래)이며,
		//    목표 날짜 - 현재 날짜 < 0일 경우, 결과값은 지난 일 수 (과거)이다.
		
		long differentDate = d2.getTime() - d1.getTime();
		System.out.println(differentDate); // long 타입 값 (밀리초 단위)
		// => 사용하기 편하도록 특정 단위로 변환 작업이 필요함
		//    ex) 밀리초 (1000) -> 초 (60) -> 분 (60) -> 시 (24) -> 일 순서로 변환
