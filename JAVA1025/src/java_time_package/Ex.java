...
//		int month = date.getMonth(); // 오류 발생
		...
		// => 주의! getMonth() 메서드는 Month 타입 객체 리턴
		int month = date.getMonthValue();
		int day = date.getDayOfMonth();
		System.out.println(year + "-" + month + "-" + day);
		
		// Month 타입 객체 활용
		// => enum타입 객체인 Month 타입으로 관리됨
		...
		// 리턴 타입이 자기 자신인 객체의 다음 메서드 차례대로 연결하여 수행 가능
		...
		// 날짜와 시각 정보 비교를 위해
		// compareTo(), isAfter(), isBefore(), isEqual() 등의 메서드 제공
		...
		//    ex) ChronoUnit.YEARS, ChronoUnit,DAYS
