...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 *   인스턴스 생성과 상관없이 클래스가 로딩되는 시점에 메모리에 로딩
		 ...
		 * 
		 * < static 변수 >
		 * - 멤버 변수에 static 키워드를 사용할 경우 클래스 (정적, static) 멤버로 취급됨
...
//		System.out.println("NormalMember : " + NormalMember.a);
		...
		// => Static이 정의되어있지 않은 인스턴스 멤버변수 a에 접근하려고 하면 에러가 발생
		...
		System.out.printf("s1.a : %d, s2.a : %d \n", s1.a, s2.a);
		System.out.printf("s1.b : %d, s2.b : %d \n", s1.b, s2.b);
		// static 멤버변수 값(a)을 s1 인스턴스에서 변경되면
...
class NormalMember{
	int a = 10; // 멤버 변수 (= 인스턴스 변수, 필드)
	int b = 20;
}

class StaticMember{
	static int a = 10; // 클래스 멤버변수 (= static, 정적 멤버변수)
	int b = 20;
}
