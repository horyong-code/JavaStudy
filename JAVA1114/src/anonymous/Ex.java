...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 * 익명 (anonymous) 객체
		 * - 클래스 이름이 없는 객체
		 * - 클래스 선언과 객체의 생성을 동시에 하기 때문에 단 한 번 사용될 수 있고
		 *   오직 하나의 객체만을 생성할 수 있는 일회성 클래스
		 * - 이름이 없기 때문에 생성자도 가질 수 없다.
		 * - 익명 객체를 만들기 위해서는 어떤 클래스를 상속하거나 인터페이스를 구현해야 함
		 * - 부모 클래스의 이름이나 구현하고자 하는 인터페이스 이름을 사용해서 정의하므로
		 *   한 번에 하나의 클래스로 상속받는 동시에 인터페이스를 구현함
		...
		NormalParent np = new NormalChild(); // 업캐스팅 (슈퍼클래스 <- 서브클래스)
		np.wake(); // 오버라이딩 된 NormalChild의 wake() 메서드 호출 [동적 바인딩]
//		np.work(); // 참조영역의 축소로 인 해 접근 불가
...
class NormalChild3 extends NormalParent implements NormalInterface{ // 동시 : extends -> implements
...
class UseExtendsClass {
	NormalParent p = new NormalChild(); // 멤버변수에 서브클래스 객체를 대입 [업캐스팅]
	
	public void method() {
		NormalParent localP = new NormalChild(); // 로컬변수에 서브클래스 객체를 대입 [업캐스팅]
	}
}
...
//		p.childMethod(); // 접근 불가
	...
	// => 익명 자식 객체에 새롭게 정의된 필드 (멤버 변수)와 메서드는 익명 자식 객체
	//    내부에서만 사용 가능, 외부에서는 접근 할 수 없음
...
// =========================================================================================
// 익명 객체를 사용한 클래스 정의
// 맴버 메서드 내에서 로컬 변수 (localP)를 선언할 때 초기값으로 익명 클래스 사용
