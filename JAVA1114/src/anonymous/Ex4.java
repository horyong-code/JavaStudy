...
public class Ex4 {
	...
	public static void main(String[] args) {
		 ...
		 * - 익명 객체는 메서드 실행이 종료되면 없어지는 것이 일반적
		 ...
		 * - 이런 문제를 해결하기 위해 자바는 컴파일 시 익명 객체에 사용하는
		 *   매개변수나 로컬변수의 값을 익명 객체 내부에 복사해두고 사용함.
		 * - 매개변수나 로컬변수가 수정되어 값이 변경되면 수정된 값과
		 *   익명 객체에 복사해둔 값이 달라지게 되어 매개변수나 로컬변수를 final로 선언할 것을 요구함
		 * - 자바 7 이전까지는 final 키워드없이 선언된 매개변수나 로컬변수를
		 *   익명 객체에서 사용 시 컴파일 에러가 발생했었음
		 *   (자바 8부터 final 키워드 없이 선언된 매개변수나 로컬변수 사용 시 에러가 발생하지 않음)
		 * - final 선언을 하지 않아도 수정될 수 없도록 final의 특성을 부여
		 */
...
//		arg1 = 20;
		// => The final local variable arg1 cannot be assigned. It must be blank and not using a compound assignment
//		arg2 = 20;
//		num1 = 30;
		// => The final local variable num1 cannot be assigned. It must be blank and not using a compound assignment
//		num2 = 30;
