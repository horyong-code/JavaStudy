...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 * } catch (예외클래스명2 변수명){
		 *		// 예외클래스명2에 해당하는 예외 발생시 처리할 코드들 
		 * } finally {
		 * 		// 예외 발생 여부와 상관없이 무조건 실행
		 *		// ex) DB 자원 반환, I/O 자원 반환 [close();]
		 * }
		 */
		
		System.out.println("프로그램 시작!");
		
		// RuntimeException 클래스 계열 (Compile Unchecked Exception 계열)
//		int num = 0;
...
//		System.out.println(arr[5]);
		...
		// java.lang.ArrayIndexOutOfBoundsException: Index 5 out of bounds for length 3 => 5번 인덱스 문제
		// at exception.Ex.main(Ex.java:49) => 49번 라인에서 예외 발생
...
//		System.out.println(str.length());
		...
		// => at exception.Ex.main(Ex.java:55) => 55번 라인에서 예외 발생
		...
		try {
			...
			System.out.println(e.getMessage()); // 예외 발생 원인 메세지를 문자열로 리턴
...
//			System.out.println("나머지 모든 예외 처리 - " + e.getMessage());
			...
			//    하위 클래스 타입부터 상위 클래스 타입 순으로 예외 처리 필수
			System.out.println("null값을 참조할 수 없음! - " + e.getMessage());
		}catch(Exception e) {
			// 위쪽의 catch 블록에서 지정되지 않은 나머지 예외들은
			// Exception이 기술된 catch 블록에서 모두 처리 가능함
			// => 단, 각 예외에 따른 처리 방법을 구분할 수 없음.
