...
public class Ex4 {
	...
	public static void main(String[] args) {
		 ...
		 *   별도의 변수없이 임시 객체 형태로 사용하는 경우가 많음
		 ...
		 * - 사용자가 직접 예외 클래스를 작성하여 발생시킬 예외에 맞는 객체를 만들어서
		 *   전달하면 예외 파악이 쉬워진다.
		 * - 사용자 정의 예외 클래스 정의 시 예외 클래스를 상속 받아 정의
		 *   (ex. Exception 클래스를 상속받은 서브 클래스 정의)
		 *   => 주로 Exception 클래스를 상속 받아 정의
	...
	public static void printScore(int score) throws Exception {
		/*
		 * 점수를 입력 받아서 학점을 계산 후 출력하는 메서드
		 * 단, 점수 범위를 0 ~ 100 사이의 정수로 한정
		 * 만약, 0 ~ 100 사이가 아닐 경우 비정상적인 점수이므로 오류 발생 필요
		 * 그러나, int형 범위를 벗어나지 않으므로 자바 기준으로는 예외가 아님!
		 * => 따라서, 원하는 시점에 특정 조건 발생 시 강제로 예외 발생
		 */
...
//			throw e;
			...
			// 위의 방법은 Exception 객체의 변수 e를 한 번만 사용함
...
// 사용자 정의 예외 클래스 작성
// - 원하는 이름의 서브 클래스 정의 후 예외 클래스를 상속받음
