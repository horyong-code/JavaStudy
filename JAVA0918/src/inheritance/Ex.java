...
public class Ex {
	...
	public static void main(String[] args) {
		/*
		 * 상속 (inheritance)
		 * - is-a 관계가 성립하는 객체 간의 관계
		 * - 슈퍼클래스 (부모)의 모든 멤버를 서브클래스 (자식)에서 물려받아
		 * 	 선언없이 사용하는 것
		 * - 슈퍼클래스 (Super class) = 부모 클래스 = 상위 클래스
		 *   서브클래스 (Sub   class) = 자식 클래스 = 하위 클래스 [서브 클래스가 슈퍼 클래스를 상속한다]
		 *   
		 * - private 접근제한자로 지정된 멤버는 상속 대상에서 제외
		 * - 생성자도 상속 대상에서 제외
		...
		// 슈퍼클래스인 Parent 클래스의 멤버도 아무런 선언없이 사용 가능함
