...
public class Ex4 {
	...
	public static void main(String[] args) {
		 ...
		 *      슈퍼클래스의 인스턴스를 생성하고 그 후에 서브클래스의 인스턴스가 생성되어 생성자 코드가 실행됨
		 */
...
class Employee4{ // = class Employee4 extends Object
	...
	public Employee4() {
		super(); // 슈퍼클래스 먼저 호출 - Object
		System.out.println("Employee4() 생성자 호출됨");	
	}
	
	// 만약, 파라미터 생성자를 정의했을 때 기본 생성자가 자동을 생성되지 않으므로
	// 서브클래스의 생성자에서 기본적으로 호출되는 슈퍼클래스의 기본 생성자가 존재하지 않을 수 있음
	// 서브 생성자에서 컴파일 에러가 발생할 수 있다
	// => 슈퍼클래스의 파라미터 생성자를 정의할 때 기본 생성자 정의 필요
...
//	}
	...
	public Manager4() {
		...
		// => 슈퍼클래스 생성자 작업이 종료된 후 서브클래스 생성자의 작업 수행
