...
public class Ex3 {
	...
	public static void main(String[] args) {
		 ...
		 * 문자형 데이터 타입 : char
		 * - 내부적으로 정수형 데이터로 처리됨
		 * - 2Byte (16bit) 크기의 범위로 사용
		 *   정수 범위 0 ~ 65535 사이의 값을 갖는다 [음수 사용 X]
		 *   (short 정수형 타입은 -32768 ~ +32767)
		 * - 문자 표현 시 1개의 문자를 작은 따옴표 ('')로 둘러싸서 표현
		 * => 작은 따옴표 내의 문자 1개가 정수형으로 변환되어 처리됨 
		 *    ex) 'A'는 정수 65로 처리됨
		 * - 유니코드 표현 방법 사용
		 ...
		 *    'A' = 65, 'a' = 97, '0' = 48		 
		 * => 10진수로 된 코드값을 사용하지만, 입력시 16진수 코드 4자리 이용 
		 *    '\' 기호와 'u'를 쓰고 16진수 XXXX 형태의 유니코드 값 표현 방법 이용
		 *    ex) \uABCD
		 */
		// char 타입 변수 ch를 선언하고 대문자 A를 저장후 출력
		char ch = 'A';
		System.out.println(ch);
		
		// char 타입 리터럴을 표기하기 위해서 작은 따옴표로 둘러싸고
		// 작은 따옴표 사이에는 반드시 1개의 문자가 포함되어야 함
...
//		   => 16진수 0041 = 10진수 65
		...
		/*
		 * 문자열을 저장하기 위한 데이터 타입 : String
		 * - 기본 데이터 타입이 아닌 참조 데이터 타입 (4Byte)
		 * - 문자 데이터를 표현하는 char 타입은 
		 *   1개의 문자만을 작은 따옴표 ('')로 둘러싸지만
		 *   문자열 데이터를 표현하는 String 타입은
		 *   0개 이상의 문자열을 큰 따옴표 ("")로 둘러싸서 표현함
		 *   => 아무 문자도 포함하지 않는 "" 문자열을 널 스트링 (Null String)이라고 함
		 *   
		 */
