...
public class Ex2 {
	...
	public static void main(String[] args) {
		...
		// 2) substring(int beginIndex, int endIndex) : 시작 인덱스 ~ 끝 - 1까지 추출
		...
		System.out.println("교육 기관은 " + s2 + "입니다.");
		...
		// => 분리된 문자열은 배열로 관리됨
		// => 분리에 사용되는 기준 문자를 구분자라고 함
		// => 구분자를 지정하는 규칙은 정규표현식을 따름
		
		// => 문자열을 "/" 기호 기준으로 분리하여 배열로 관리 가능
		String[] subjectArr = s3.split("/");
		// => 문자열을 "/" 기호 기준으로 분리하여 String[] 배열타입으로 리턴
...
//		}
		...
		// 정규 표현식 규칙에 따라 . 기호를 구분자로 지정해야할 경우 \\. 사용
		String[] strArr = s4.split("\\.");
		for(int i = 0; i < strArr.length; i++) {
			System.out.println(strArr[i]);
		}
		...
		// String.format()
		// 특장 문자열을 형식 지정 문자 (%?)와 결합하여 형식을 갖춘 문자열로 리턴
		// => printf() 메서드와 동일한 형식 지정문자 동일
		String name = "홍길동";
		int age = 20;
		double height = 180.7;
