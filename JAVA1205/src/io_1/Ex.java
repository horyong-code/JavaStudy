...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 *   => 출발지 노드 -> 도착지 노드
		 * - 자바는 다양한 입출력 장치와 무관하고 일관성있게 프로그램을 구현할 수 있도록
		 *   일종의 가상 통로인 스트림을 제공
		 *   
		 * [ 입력 스트림과 출력 스트림 ]
		 * 1. 입력 스트림
		 ...
		 * 
		 * 2. 보조 스트림
		 * => 직접 읽고 쓰는 기능은 없고, 항상 다른 스트림을 포함하여 생성됨 [Stream + Reader + Writer + Buffer]
...
//		InputStream is = null;
//		// => finally 블록에서도 사용하기 위해 trey문 위쪽에 선언
//		
//		try {
//			is = System.in;
...
//			int n = is.read();
//			// => read() 메서드는 try ~ catch문을 사용하여 예외 처리가 필수!
//			// => 입력 스트림 데이터 중 1Byte만큼의 데이터를 읽어서
...
//		}
		 ...
		 *   close()메서드 호출을 통해 사용 중인 자원을 반환해야 하며
		 *   자원이 반환되지 않으면 반복적인 자원 요청으로 인해 자원이 고갈되어
		 *   더 이상 다른 사용자의 작업 요청을 수행할 수 없게 된다!
		 *   => 예외 발생 여부와 관계없이 finally 블록 내에서 자원 반환 코드 기술
		 * - try ~ with resources 구문 내 try문에서 반환할 자원을 갖는 객체를 생성하고
		 *   try ~ catch 블록이 끝나면 자동으로 자원을 반환해주도록 함
		 * 
		 * < 기본 문법 >
...
//			e.printStackTrace();
//		}
		// => 별도의 close() 메서드를 호출하지 않아도 자동으로 자원이 반환 됨
		...
		try(InputStream is = System.in;) {
			int n = is.read();
			// 반복문을 사용하여 더 이상 읽어올 데이터가 없을 때 (-)까지 입력을 처리
			while(n != -1) {
