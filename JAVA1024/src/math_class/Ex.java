...
public class Ex {
	...
	public static void main(String[] args) {
		...
		// => long 타입 결과값을 int 타입 1000으로 나누면 결과값이 long이므로
		//    반드시 둘 중 하나라도 double 타입 실수로 연산을 수행해야함
		System.out.println(Math.round(dNum * 1000) / 1000.0);
		
		// ======================================================================
		// 참고! 특정 위치의 숫자까지만 남기고 나머지는 제거해야 할 경우
		// ex) 3.141592의 소수점 넷째 자리만 남길 경우 3.1415
		
		// 1) 원하는 자리의 숫자까지 정수 자리에 위치하도록 곱함
		//     3.141592 -> 31415.92
		System.out.println(dNum * Math.pow(10, (5-1)));
		
		// 2) 소수 부분의 숫자를 제거하기 위해 정수화 진행
		//     31415.92 -> 31415
		System.out.println((int) (dNum * Math.pow(10, (5-1))));
		
		// 3) 다시 원래 숫자로 복원하기 위해 처음에 곱한 값으로 다시 나눔
		//     => 주의! 소수점 표현을 위해 double 타입 실수로 나누어야 함!
		//     31415 -> 3.1415
		System.out.println((int) (dNum * Math.pow(10, (5-1))) / Math.pow(10, (5-1)));
