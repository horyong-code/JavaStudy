...
public class Ex2 {
	...
	public static void main(String[] args) {
		 ...
		 *   하나의 슈퍼클래스 타입으로 여러 서브 클래스 타입 인스턴스를 다룰 수 있다
		 */
		...
		//    Shape 타입 (s)으로 세 개의 인스턴스를 컨트롤 가능
		// Circle -> Shape 업캐스팅
		Shape s = new Circle();
...
//		sArr[2] = new Triangle(); // Triangle -> Shape 업캐스팅
		...
		// 메서드에 다형성 활용
//		Ex2 ex2 = new Ex2();
//		ex2.polymorphismDraw(sArr);
		// => static이 설정되지 않은 '인스턴스 메서드'를 호출하는 경우
		//    먼저 인스턴스를 생성하고 참조변수를 통해 호출할 수 있음
		...
		// => 클래스명이 동일한 static 메서드의 호출인 경우 클래스명 생략 가능
		
		// 2. 메서드 파라미터로 인스턴스를 직접 전달하는 형태
...
class Shape{
	// 여러 도형의 공통점인 '그리다' 기능하는 draw() 메서드 정의
	public void draw() {
