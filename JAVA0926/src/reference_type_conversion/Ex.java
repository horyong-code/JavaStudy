...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 * - 컴파일러에 의해 자동 형변환이 일어남
		 *   = 별도의 형변환 연산자를 사용하지 않아도 변환이 가능
		 * - 참조 가능한 영역이 축소됨
...
//		p.childPrn(); // => The method childPrn() is undefined for the type Parent
		...
		// Parent 타입 레퍼런스 p로 접근 가능한 메서드 : 1개
...
//		// => 슈퍼클래스 타입은 서브클래스 타입으로 자동 형변환 불가
//		// => 반드시 형변환 연산자를 통해 명시적 (강제) 형변환이 필수
//		c2 = (Child) p2; // Parent -> Child 타입으로 강제 형변환 = 다운 캐스팅
//	
...
//		c2.childPrn(); // 실제 Parent 인스턴스에 존재하지 않는 메서드
		...
		//    다운캐스팅이 기본적으로 불가능함
		//    메서드 호출 시점이 아닌 다운 캐스팅 시점에서 오류가 발생함
		
		// 기본적으로 자바는 다운캐스팅을 허용하지 않음
