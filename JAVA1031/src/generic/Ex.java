...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 * - 주로, Collection API의 클래스들이 제네릭이 적용되어있으므로
...
//		nic.data = "홍길동"; // 문자열 저장 불가
		// => 여러 데이터타입 데이터를 모두 저장하려면 최소한 Object 타입 변수로 선언해야함
...
//		System.out.println("이름 : " + name);
		...
		//    (int 대신 Integer, char 대신 Character 사용 [Wrapper 클래스])
...
//		gc.setMember("홍길동"); // 정수 타입 외에 모두 컴파일 에러 발생!
		// => The method setMember(Integer) in the type GenericClass<Integer> is not applicable for the arguments (String)
...
class GenericClass<T> {
	...
	T member; // 멤버변수 member의 데이터타입이 T로 지정됨 (실제 데이터 타입 X)
...
class NormalObjectClass {
	...
	// setData(1) -> Object 참조형 : 1 기본형
	// 1) int (기본형) -> Integer (참조형) : 오토 박싱
	// 2) Integer -> Object : 업캐스팅
}