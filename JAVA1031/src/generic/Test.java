...
public class Test {
	...
	public static void main(String[] args) {
		...
		// 1. Object[] 배열 (또는 Person[] 배열)을 통해서 관리
		...
		for(int i = 0; i < objArr.length; i++) {
			// Object[] 배열에 저장된 객체를 직접 다룰 경우 참조영역 축소
...
//			Person p = objArr[i]; // 다운캐스팅 필요
		 ...
		 *    Person 객체 여러 개를 관리할 경우
		 *    => 배열의 단점인 크기 불변을 해결하게 되므로 객체 추가가 자유로움
		 * 1) 제네릭을 사용하지 않을 경우
		 * - 파라미터 또는 리턴타입의 데이터타입이 Object 타입이 되어 다양한 객체를 저장 가능하게 됨
		...
		List list = new ArrayList(); // 업캐스팅 : 다형성 -> 통일성 향상 [ArrayList list = new ArrayList();] 
		...
		// 문자열 추가
		list.add("강호룡"); // Object 타입이므로 Person이 아닌 타입도 추가 가능
		// => Person 객체 형태로 사용하는 시점에서 문제가 발생할 수 있다!
		
//		// 일반 for문을 통한 객체 출력
//		for(int i = 0; i < list.size(); i++) {
////			Person p = list.get(i);
//			// => 제네틱 타입을 사용하지 않아 list는 Object로 되어 있다.
//			// => Object 타입을 Person 타입으로 대입하려고 하면 컴파일 에러가 나며
//			//    다운캐스팅이 필요함을 알 수 있다.
////			Person p = (Person) list.get(i);
////			System.out.println(p);
//			// 실행을 시키면 에러 발생
//			// => java.lang.ClassCastException: class java.lang.String cannot be cast to class generic.Person
//			// => String 형태는 Person 객체로 변경할 수 없음을 알 수 있다
//			// 위의 코드 중 "list.add("강호룡")";을 주석 처리하고 다시 실행하면 오류가 없어짐
//			
//			// Person 타입으로 가져오기 전에 타입 판별 후 형변환 수행
...
//		}
		...
		// 향상된 for문 사용 시
		// 우변의 list 객체에서 꺼낸 객체를 저장할 변수 좌변에 선언 (Object)
		for(Object o : list) {
		...
		// 데이터 판별 과정에서 오류 (컴파일 에러)가 발생하게 된다
//		list2.add("강호룡"); // 컴파일 에러 발생
